...
<div class="container py-5" style="max-width:760px">
    <div class="card shadow-sm border-0">
        <div class="card-body p-4 p-md-5 text-center">
            ...
            <a href="{{ route('order.success', $order) }}" class="btn btn-link mt-2">Kembali ke detail pesanan</a>
    ...
    try {
        ...
        button.addEventListener('click', () => {
            window.snap.pay(token, {
                onSuccess: () => window.location.href = @json(route('order.success', $order)),
                onPending: () => window.location.href = @json(route('order.success', $order)),
                onError: () => {
                    error.textContent = 'Pembayaran gagal. Silakan coba lagi.';
